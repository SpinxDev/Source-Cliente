...
void CECHostPlayer::OnMsgHstPickupItem(const ECMSG& Msg)
{
	...
	if (pItem)
	{
		if (pItem->IsEquipment() || pItem->GetClassID() == CECIvtrItem::ICID_TASKNMMATTER ||
			...
			pItem->GetClassID() == CECIvtrItem::ICID_ITEMPACKAGEBYPROFESSION )
...
void CECHostPlayer::OnMsgHstItemOperation(const ECMSG& Msg)
{
	...
	case EQUIP_ITEM:
	{
		...
		if (pCmd->index_equip == EQUIPIVTR_ASTROLABE)
		{
			CDlgAstrolabe* pDlg = dynamic_cast<CDlgAstrolabe*>(g_pGame->GetGameRun()->GetUIManager()->GetInGameUIMan()->GetDialog("Win_Astrolabe"));
			if (pDlg && pDlg->IsShow()) 
				pDlg->Refresh();
		}
		break;
...
{
	using namespace S2C;
	astrolabe_operate_result* pCmd = (astrolabe_operate_result*)Msg.dwParam1;
	SetAstrolabeOperation(*pCmd);
}