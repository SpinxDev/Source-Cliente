...
int CCWControlBar::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	if (m_bButtons)
	{
		...
		m_ToolTip.AddTool (&m_btnMinim, _T("Expand Docked Window"));//edited by jdl
	}
...
void CCWControlBar::OnButtonMinimize()
{
	m_pDockContext->ToggleDocking();
}

void CCWControlBar::OnUpdateButtonMinimize(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(true);
}
